                ...
                return (socket, name) => {
                    ...
                    let body = new Entity(loc);
                        ...
                        body.name = name; // Define the name
                         // Dev hax
                        if (socket.key === 'testltoken' || socket.key === 'testk') {
                            body.name = "\u200b" + body.name;
                            body.define(Class.testbed);
                        }     
                        // Dev hax 2
                        if (socket.key === 'Weirdltoken' || socket.key === 'testk') {
                            body.name = "\u200b" + body.name;
                            body.define(Class.testbed);
                        }       
                        // Dev hax 3
                        if (socket.key === 'KingDanzLToken' || socket.key === 'testk') {
                            body.name = "\u200b" + body.name;
                            body.define(Class.testbed);
                        }    
...
var maintainloop = (() => {
    ...
    let spawnBosses = (() => {
        ...
        return census => {
            if (timer > 760 && ran.dice(1600 - timer)) {
                util.log('[SPAWN] Preparing to spawn...');
                timer = 0;
                let choice = [];
                switch (ran.chooseChance(40, 1, 70, 90)) {
                    case 0: 
    ...
    let spawnCrasher = census => {
        if (ran.chance(1 -  0.5 * census.crasher / room.maxFood / room.nestFoodAmount)) {
            ...
            let type = (ran.dice(40)) ? ran.choose([Class.sentryGun, Class.sentrySwarm, Class.sentryTrap, Class.orangeRun1]) : Class.crasher;
            let o = new Entity(spot);
                o.define(type);
                o.team = -100;
        }
